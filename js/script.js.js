...
function verificarAprovacao() {
    ...
    if (timer && Date.now() - timer > 60000) {
        database.ref('solicitacoes').once('value', (snapshot) => {
            if (snapshot.val() && snapshot.val().status === 'pendente') {
...
function aprovarTecnico() {
    database.ref('solicitacoes').update({ status: 'aprovado' });
    localStorage.setItem('tecnicoAprovado', 'sim');
    fecharModalAprovacao();
}

function rejeitarTecnico() {
    database.ref('solicitacoes').update({ status: 'rejeitado' });
    fecharModalAprovacao();
}
...
function verificarSolicitacaoTecnico() {
    database.ref('solicitacoes').on('value', (snapshot) => {
        ...
        if (solicitacao && solicitacao.status === 'pendente') {
            const tempoRestante = Math.ceil((60000 - (Date.now() - solicitacao.timestamp)) / 1000);
            if (tempoRestante > 0 && !document.getElementById('modalAprovacao')) {
